...
pub fn truncated_file_auto_repair_test() {
  ...
  let result = set.open_with(path, slate.AutoRepair)
  case result {
    Ok(table2) -> {
...
pub fn no_repair_rejects_corrupted_file_test() {
  ...
  let result = set.open_with(path, slate.NoRepair)
  case result {
    Error(_) -> Nil
    Ok(table2) -> {
...
pub fn force_repair_on_corrupted_file_test() {
  ...
  let result = set.open_with(path, slate.ForceRepair)
  case result {
    Ok(table2) -> {
