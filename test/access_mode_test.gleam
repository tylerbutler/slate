...
pub fn set_readonly_nonexistent_file_fails_test() {
  ...
  let result = set.open_with_access(path, slate.AutoRepair, slate.ReadOnly)
  case result {
    Error(_) -> Nil
    Ok(table) -> {
      let assert Ok(Nil) = set.close(table)
...
pub fn bag_readonly_lookup_test() {
  ...
  let assert Ok(ro) =
    bag.open_with_access(path, slate.AutoRepair, slate.ReadOnly)
...
pub fn dupbag_readonly_lookup_test() {
  ...
  let assert Ok(ro) =
    duplicate_bag.open_with_access(path, slate.AutoRepair, slate.ReadOnly)
...
pub fn set_readwrite_mode_test() {
  ...
  let assert Ok(table) =
    set.open_with_access(path, slate.AutoRepair, slate.ReadWrite)
  let assert Ok(Nil) = set.insert(table, "key", "val")
